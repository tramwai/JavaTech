...
public class Practice04_MinMaxValues {
    public static void main(String[] args) {
        ...
        System.out.println(nthMin(new ArrayList<>(Arrays.asList(10, 100, 123, 507, 25)), 4)); // 123
    ...
    /**
    Find the max from given numbers
    [10, 100, 123, 507, 25] -> 507
     [12] -> 12
     [] -> null or NoSuchElementException(must ask)
     */
    ...
    /**
     Find the min from given numbers
     [10, 100, 123, 507, 25] -> 507
     [12] -> 12
     [] -> null or NoSuchElementException(must ask)
     */
...
//    }
    ...
    public static Integer secondMax(Integer[] arr){
       ...
       set.pollLast(); // max is removed
       return set.last();
    }


    /**
     Find the second min from given numbers
    ...
    public static Integer secondMin(Integer[] arr){
        ...
        TreeSet<Integer> set = new TreeSet<>(Arrays.asList(arr));
        if (set.size() < 2) return null;
        set.pollFirst(); // min is removed
        return set.first();
    }

    /**
     Find the nth max from given numbers
     [10, 100, 123, 507, 25], 4th max -> 25
     [10, 100, 123, 507, 25], 3rd max -> 10
     [10, 100], 3rd max -> null
    ...
    public static Integer nthMax(List<Integer> numbers, int nth){
        ...
        // nth - 1 pollLast() then return last
    ...
    /**
     Find the nth min from given numbers
     [10, 100, 123, 507, 25], 4th max -> 25
     [10, 100, 123, 507, 25], 3rd max -> 10
     [10, 100], 3rd max -> null
    ...
    public static Integer nthMin(List<Integer> numbers, int nth){
        TreeSet<Integer> set = new TreeSet<>(numbers);
        if(set.size() < nth) return null;
        // nth - 1 pollLast() then return first
        for (int i = 1; i <= nth - 1; i++) {
            set.pollFirst();
        }
        return set.first();
